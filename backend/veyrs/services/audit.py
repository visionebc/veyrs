...
def record(
    ...
    *,
    ...
    # A SAVEPOINT, not a bare flush: a rejected INSERT (RLS WITH CHECK, a
    # constraint) otherwise leaves the whole session needing a rollback, and the
    # request that was only trying to leave a trace fails on its next query.
    try:
