...
# Audit trails whose tenant column is nullable in the model but which are still
# strictly isolated. A nullable tenant would otherwise mean "platform-level row,
# left to the app layer" -- and an audit trail readable across tenants leaks who
# changed what in every other organization. Every writer passes a tenant
# (`services/audit.py`); production has forced these since before 0.32, and a
# fresh install was the laxer of the two until 0.32.3.
RLS_TABLES_STRICT_NULLABLE = {"audit_log", "ai_audit_log"}
    ...
    with engine.begin() as conn:
        for name, table in Base.metadata.tables.items():
            ...
            if (table.c.organization_id.nullable and name not in RLS_TABLES_PREAUTH
                    and name not in RLS_TABLES_STRICT_NULLABLE):
