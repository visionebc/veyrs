...
## 0.32.3 - 2026-09-25 - the audit trails are tenant-isolated on every install

**Schema/RLS change.** `init-db` now forces row level security on `audit_log`
and `ai_audit_log` (`RLS_TABLES_STRICT_NULLABLE` in `cli.py`). Their tenant
column is nullable in the models, so a fresh install used to skip them: 68 of
87 tables forced, against 70 on production deployments, which already carried
the strict policy. An audit trail readable across tenants tells one
organization who changed what in every other one. Existing installs pick it up
on the next `init-db`, which is idempotent and touches no rows.

Making that safe needed three code fixes, and two of them were live bugs on any
deployment with the strict policy:

- **Audit writes run in a SAVEPOINT** (`services/audit.py`). A write the policy
  rejected used to leave the session needing a rollback, so the request it was
  only meant to record failed on its next query. That broke the module's own
  promise that "audit must not break the request". Now the row is logged as
  `audit write failed` and dropped, and the request carries on.
- **First login of a directory-provisioned (JIT) user** wrote its
  `user.provisioned_from_directory` audit row with no tenant and wrote its
  `user_roles` grants before any tenant was bound. Both fail WITH CHECK under
  strict RLS. The organization whose directory just vouched for the account is
  now bound first.
- **Directory role sync on login** (`sync_roles_on_login`) ran unbound as well,
  so on a user's second directory login the existing grants read back as zero
  rows and were inserted again, which is a unique-constraint 500. Same fix.

No production tenant has LDAP enabled today, so the last two had not fired.
`tests/test_audit_rls.py` (7 tests) covers the policy, isolation across
tenants, a rejected write leaving the session usable, and both directory paths.
It refuses to run as a role that bypasses RLS.

## 0.32.2 - 2026-09-25 - emphasis the docs renderer dropped or invented

Documentation renderer only (`site/build.py`, shared by the docs site and the
product site). No application, schema or installer change.

Bold that contained italics (`**from the last *successful* run**`) or sat
inside a word (`**V**isibility`) was left on the page as literal asterisks:
9 places across the docs. Worse, asterisks inside code spans were read as
emphasis, so `*:*` rendered as an italic colon: 8 code spans, 4 of them in the
Docker Compose manual, i.e. commands a reader would copy. Code spans are now
set aside before emphasis is applied. Three new tests, all failing on the
previous renderer; one sweeps every document in `docs/`.

## 0.32.1 - 2026-09-25 - the installer on the front page, and docs links that work from outside

Documentation and product site only. No application, schema or installer change.

`README.md`, `INSTALL.md` and `docker/README.md` now open with the guided
installer: download `veyrs-setup.sh` from the latest release, check it against
its `.sha256`, `--check`, then run it. In 0.32.0 only the Docker Compose manual
mentioned it, so a visitor to the repository saw `install.sh` and
`veyrs-docker.sh` but not the installer the release was built around.

The product site (`site/web/build_web.py`) gains `--public`. Every
documentation link on the internet-facing site pointed at the internal docs
host, which does not resolve outside the LAN: 16 of the 18 cards, the footer and the
security and availability pages were dead links for a public visitor. With
`--public` they point at the Markdown on the public mirror instead; without it
(the LAN site) nothing changes. The "ADR index" link pointed at `/adr/`, which
the docs site answers with 403 on the LAN too; it now opens the ADR README.
Two new tests fail on the previous builder.
